\documentclass[aspectratio=169, 10pt]{beamer}
\usetheme{Madrid}
\usecolortheme{default}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, positioning}
\usepackage{booktabs}
\usepackage{listings}
\usepackage{xcolor}

\lstdefinestyle{cstyle}{
    language=C,
    basicstyle=\ttfamily\scriptsize,
    backgroundcolor=\color{black!5},
    keywordstyle=\color{blue}\bfseries,
    commentstyle=\color{green!50!black},
    stringstyle=\color{red},
    frame=single,
    breaklines=true,
}
\lstset{style=cstyle}

\title[Server HTTP Concorrente POSIX]{Progetto 1: Server HTTP Concorrente}
\subtitle{Gestione Prenotazione Aule Studio in C}
\author[Mantineo, Castorina]{Massimo Mantineo (541924) \and Pierluca Tino Castorina (545101)}
\institute[UniMe]{Università degli Studi di Messina \\ Dipartimento MIFT - Laurea in Informatica}
\date{Anno Accademico 2024/2025}

\begin{document}

\begin{frame}
    \titlepage
\end{frame}

\begin{frame}{1. Obiettivi del Progetto}
    \begin{block}{Il Problema}
        Implementare un server per la gestione di prenotazioni in grado di servire simultaneamente molteplici client, garantendo la coerenza dei dati e riducendo l'overhead computazionale di sistema.
    \end{block}
    
    \vspace{0.3cm}
    
    \textbf{Le Soluzioni Adottate:}
    \begin{itemize}
        \item \textbf{C Nativo \& POSIX:} Utilizzo delle \textit{system call} di basso livello anziché di framework moderni (Node.js, Spring) per massimizzare il controllo delle risorse.
        \item \textbf{Architettura a Thread Pool:} Sostituzione del pattern classico basato su \texttt{fork()} con un pool di worker thread pre-allocati.
        \item \textbf{Sincronizzazione Esplicita:} Impiego combinato di Semafori (per la comunicazione di rete) e Mutex (per l'accesso in mutua esclusione al database in memoria).
    \end{itemize}
\end{frame}

\begin{frame}{2. Architettura a Thread Pool}
    \centering
    \resizebox{0.9\textwidth}{!}{
    \begin{tikzpicture}[
        node distance=1.5cm and 1.5cm,
        box/.style={draw, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=blue!10, font=\sffamily},
        database/.style={draw, cylinder, shape aspect=0.25, shape border rotate=90, minimum width=2cm, minimum height=2cm, align=center, fill=green!10, font=\sffamily\bfseries},
        arrow/.style={-Latex, thick, draw=black!70},
        dashed_arrow/.style={-Latex, dashed, thick, draw=black!50}
    ]

    \node[box, fill=orange!10, font=\sffamily\bfseries] (client) {Client HTTP};
    \node[box, below=1cm of client] (main) {\textbf{Dispatcher}\\ \small \texttt{accept()}};
    \node[box, right=of main, fill=gray!10] (queue) {\textbf{Job Queue} \\ \small (Coda FIFO)};
    
    \node[box, above right=0.1cm and 1.5cm of queue] (worker1) {Worker Thread 1};
    \node[box, right=1.5cm of queue] (worker2) {Worker Thread 2};
    \node[box, below right=0.1cm and 1.5cm of queue] (worker3) {Worker Thread N};
    
    \node[database, right=1.5cm of worker2] (rm) {Database\\In-Memory};

    \draw[arrow] (client) -- node[left, font=\scriptsize] {TCP 8080} (main);
    \draw[arrow] (main) -- node[above, font=\scriptsize] {\texttt{sem\_post()}} (queue);
    
    \draw[dashed_arrow] (queue) -- node[above left, font=\scriptsize] {\texttt{sem\_wait()}} (worker1.west);
    \draw[dashed_arrow] (queue) -- (worker2.west);
    \draw[dashed_arrow] (queue) -- (worker3.west);
    
    \draw[arrow] (worker1.east) -- (rm.west);
    \draw[arrow] (worker2.east) -- node[above, font=\scriptsize] {\texttt{mutex\_lock()}} (rm.west);
    \draw[arrow] (worker3.east) -- (rm.west);
    
    \end{tikzpicture}
    }
\end{frame}

\begin{frame}[fragile]{3. Il Pattern Produttore-Consumatore}
    Per interfacciare in modo sicuro la rete (Main Thread) e l'esecuzione (Worker Threads), si sfrutta una \textbf{Job Queue} regolata da Semafori, prevenendo i colli di bottiglia e il \textit{busy-waiting}.
    
    \begin{columns}
        \begin{column}{0.48\textwidth}
            \textbf{Dispatcher (Produttore)}
\begin{lstlisting}[style=cstyle]
// Accettazione connessione client
client_sock = accept(server_fd, ...);
task->socket = client_sock;

// Aggiunta in coda critica
pthread_mutex_lock(&pool->queue_mutex);
pool->queue_tail->next = job;
pthread_mutex_unlock(&pool->queue_mutex);

// Incremento del semaforo
sem_post(&pool->queue_sem);
\end{lstlisting}
        \end{column}
        
        \begin{column}{0.48\textwidth}
            \textbf{Worker (Consumatore)}
\begin{lstlisting}[style=cstyle]
while (1) {
    // Il thread e' in sleep se la coda e' vuota
    sem_wait(&pool->queue_sem);
    
    // Estrazione in mutua esclusione
    pthread_mutex_lock(&pool->queue_mutex);
    job_t* job = pool->queue_head;
    // ...
    pthread_mutex_unlock(&pool->queue_mutex);
    
    // Parsing ed esecuzione
    handle_client(job);
}
\end{lstlisting}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}[fragile]{4. Thread Safety sulle Prenotazioni}
    Tutte le chiamate API che leggono o modificano la lista delle aule sono protette mediante un costrutto \textbf{Mutex} centralizzato. Questo garantisce l'isolamento transazionale ed evita corruzioni (Data Race).
    
\begin{lstlisting}[style=cstyle]
int rm_create(resource_manager_t* rm, const char* room, const char* student) {
    pthread_mutex_lock(&rm->mutex); // ACQUISIZIONE LOCK
    
    if (rm->count >= MAX_RESERVATIONS) {
        pthread_mutex_unlock(&rm->mutex);
        return -1;
    }
    
    // Inserimento sicuro dei dati nella memoria condivisa
    strncpy(rm->reservations[rm->count].room, room, 127);
    strncpy(rm->reservations[rm->count].student, student, 127);
    rm->count++;
    
    pthread_mutex_unlock(&rm->mutex); // RILASCIO LOCK
    return rm->count - 1;
}
\end{lstlisting}
\end{frame}

\begin{frame}{5. Specifiche API REST (CRUD)}
    L'applicativo funge da Web Server backend e riceve/invia stringhe formattate nativamente in formato \textbf{JSON}.
    
    \vspace{0.5cm}
    \centering
    \renewcommand{\arraystretch}{1.3}
    \resizebox{0.95\textwidth}{!}{
    \begin{tabular}{@{}lllc@{}}
        \toprule
        \textbf{Metodo} & \textbf{Endpoint} & \textbf{Azione Interna} & \textbf{Status Code} \\
        \midrule
        \texttt{GET} & \texttt{/reservations} & Serializzazione e lettura in sola lettura & \texttt{200 OK} \\
        \texttt{POST} & \texttt{/reservations} & Validazione Body URL-Encoded, Aggiunta Struct & \texttt{201 Created} \\
        \texttt{PUT} & \texttt{/reservations/\{id\}} & Ricerca indice array e sovrascrittura valori & \texttt{200 OK} \\
        \texttt{DELETE} & \texttt{/reservations/\{id\}} & Rimozione elemento e traslazione a sinistra (shift) & \texttt{204 No Content} \\
        \bottomrule
    \end{tabular}
    }
    
    \vspace{0.4cm}
    \begin{block}{Autenticazione}
        Le rotte di inserimento e modifica richiedono un header \texttt{Authorization: Basic} che viene decodificato dal parser nativo in C e confrontato prima di rilasciare l'accesso.
    \end{block}
\end{frame}

\begin{frame}{6. Ottimizzazioni Architetturali}
    \begin{block}{Lock a Grana Fine (Fine-Grained Locking)}
        Nelle architetture REST le letture superano enormemente le scritture.
        Sostituire il \texttt{pthread\_mutex\_t} con un \textbf{Read-Write Lock} (\texttt{pthread\_rwlock\_t}) consente a infiniti Worker Thread di leggere simultaneamente lo stato, bloccando l'accesso in mutua esclusione solo durante le scritture (\texttt{POST}/\texttt{DELETE}).
    \end{block}
    
    \vspace{0.3cm}
    
    \begin{block}{De-frammentazione della Memoria Dinamica}
        Poiché il C è sprovvisto di Garbage Collector, la rimozione logica di una prenotazione (\texttt{DELETE}) espone al rischio di frammentazione del vettore.
        L'implementazione impiega uno \textbf{shift a sinistra} tramite \texttt{memmove()} all'interno dell'area di memoria esclusiva, garantendo la compattezza continua dell'array dinamico.
    \end{block}
\end{frame}

\begin{frame}{7. Sicurezza e Gestione Vulnerabilità}
    \textbf{Mitigazione delle criticità intrinseche in ambiente C/POSIX:}
    \begin{itemize}
        \item \textbf{Socket Leaks:} La tracciatura fallace dei \textit{File Descriptor} sfocia nell'esaurimento delle risorse limite di sistema (\texttt{ulimit}). Il server impone la dismissione sicura \texttt{close(fd)} per ogni ciclo di Worker terminante.
        \item \textbf{Buffer Overflows:} I Worker utilizzano varianti sicure con limiti massimi (\texttt{snprintf()}, \texttt{strncpy()}) per evitare iniezioni e corruzione di memoria, rifiutando pacchetti anomali con \texttt{413 Payload Too Large}.
        \item \textbf{Interruzione SIGPIPE:} La chiusura brutale di un socket da parte del client durante l'erogazione causa la fatale disconnessione del demone tramite \texttt{SIGPIPE}. L'impiego del flag POSIX \texttt{MSG\_NOSIGNAL} previene l'interruzione propagando un errore catturabile dai thread.
    \end{itemize}
\end{frame}

\begin{frame}{8. Analisi e Load Testing}
    Il progetto è stato collaudato sfruttando lo strumento standard \textbf{Apache Bench (ab)}.
    La simulazione ha previsto: \textbf{10.000 richieste totali} con un livello di concorrenza di \textbf{100 thread paralleli}.
    
    \vspace{0.3cm}
    
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{block}{Metriche di Prestazione}
                \begin{itemize}
                    \item \textbf{Total Requests:} 10.000
                    \item \textbf{Concorrenza Esterna:} 100
                    \item \textbf{Success Rate:} 100\% (0 Errori)
                    \item \textbf{Throughput:} $\sim 33.229$ Req/Sec
                \end{itemize}
            \end{block}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Interpretazione Architetturale} \\
            L'assenza totale di connessioni droppate dimostra che la coda semaforica regge il carico e previene i fallimenti TCP. 
            Inoltre, il tempo di risposta mediano di appena \textbf{3.0 ms} per richiesta conferma la leggerezza dei thread worker e la corretta implementazione dei lock Mutex sulle risorse condivise.
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{9. Conclusioni e Sviluppi Futuri}
    L'implementazione in linguaggio C di un Thread Pool associato a socket BSD si è dimostrata una soluzione \textit{memory-safe} ad altissime prestazioni per la costruzione di architetture HTTP concorrenti.

    \vspace{0.4cm}
    \textbf{Sviluppi per Ambienti di Produzione:}
    \begin{enumerate}
        \item \textbf{Persistenza dei Dati}: Passaggio dalla struttura in RAM ad un database relazionale leggero (come \texttt{SQLite3}) per mantenere lo stato dopo il riavvio del processo.
        \item \textbf{I/O Asincrono (\texttt{epoll})}: Sostituzione delle chiamate bloccanti \texttt{read()} con un loop \textit{Event-Driven} nativo Linux per supportare il limite dei 10.000 Socket attivi.
        \item \textbf{Cifratura}: Implementazione del protocollo \textit{TLS/SSL} tramite librerie come \textit{OpenSSL} per esporre API sicure (HTTPS).
    \end{enumerate}
    
    \vspace{0.5cm}
    \centering
    \Large \textbf{Grazie per l'attenzione.}
\end{frame}

\end{document}
