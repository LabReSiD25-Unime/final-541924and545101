...
\begin{sloppypar}
\textbf{Ottimizzazione tramite Lock a Grana Fine (Fine-Grained Locking)}:
L'adozione di un unico Mutex globale per tutto il database in memoria penalizza il parallelismo, forzando le transazioni REST ad accodarsi artificialmente. Un'evoluzione notevole di questa architettura risiede nella sostituzione del \texttt{pthread\_mutex\_t} con un \texttt{pthread\_rwlock\_t} (Read-Write Lock). Le architetture REST mostrano intrinsecamente un rapporto sbilanciato a favore delle letture rispetto alle scritture. Un lock di tipo RW consente a infiniti Worker Thread di invocare blocchi in lettura (\texttt{pthread\_rwlock\_rdlock()}), processando parallelamente centinaia di richieste \texttt{GET}. Nel momento in cui un pacchetto \texttt{POST} o \texttt{DELETE} richiede l'accesso al dato, il relativo thread chiede il lock in scrittura (\texttt{pthread\_rwlock\_wrlock()}); il sistema attenderà il completamento delle letture in corso e garantirà accesso esclusivo alla mutazione della memoria.

\textbf{Integrità della Memoria Dinamica e De-frammentazione (DELETE)}:
L'operazione di DELETE impone considerazioni critiche nei linguaggi non provvisti di Garbage Collector automatico. Lasciare un campo azzerato all'interno di un vettore genera rapida frammentazione. I programmatori ricorrono alla transizione di memoria con shift a sinistra, sfruttando routine di libreria altamente ottimizzate a livello assembly come \texttt{memmove()} (preferita a \texttt{memcpy()} in quanto garantisce la sicurezza in caso di sovrapposizione delle aree di memoria).
\end{sloppypar}
